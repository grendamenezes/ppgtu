...
def mensal_bar(mes,tipo,ano,link): #ex: 1,Presencial
...
def diario_bar (dia,tipo): #ex: 10/01/2022,Remoto
	df         = pd.read_excel('atividade.xlsx')
	df['DATA'] = pd.to_datetime(df['DATA'], dayfirst=True)
...
def mensal_line(mes,tipo,ano,link): #ex: 1,Remoto
	year       = ano
	month      = mes
...
def mensal_todos(mes,ano,link): #ex: 1
	df         = pd.read_excel('atividade.xlsx')
	df['DATA'] = pd.to_datetime(df['DATA'], dayfirst=True)
	df         = df.loc[df['DATA'].dt.month == mes]
	df         = df.loc[df['DATA'].dt.year == ano]
